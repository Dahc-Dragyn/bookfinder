...
// Aggressive bots identified in your logs (Amazonbot) and common AI scrapers.
// Blocking these here prevents Cloud Run from scaling up due to high CPU/Bandwidth.
...
export async function GET(req: Request) {
  // 1. SHIELD: Check User-Agent for aggressive bots immediately
  const userAgent = req.headers.get("user-agent") || "";
  if (BANNED_BOT_STRINGS.some(bot => userAgent.includes(bot))) {
    console.warn(`[Proxy] Blocked bot request from: ${userAgent}`);
    return new NextResponse("Bot traffic not permitted on image proxy", { status: 403 });
  }
  ...
  try {
    ...
    // Cache-Control helps "cheapness" by letting the browser/CDN store the image.
